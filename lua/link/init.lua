...
---apply link colorscheme
---@param configs linkConfig
local function apply(configs)
   local colors = configs.colors
   apply_term_colors(colors)
   local groups = require("link.groups").setup(configs)
...
local function get_configs()
   ...
   if g.colors_name == 'link-soft' then
      configs.theme = 'link-soft'
      configs.colors = require('link.palette-soft')
   elseif g.colors_name == 'link' then
      configs.theme = 'link'
      configs.colors = require('link.palette')
   end
...
---setup link colorscheme
---@param configs linkConfig?
local function setup(configs)
   if type(configs) == "table" then
      user_configs = configs --[[@as linkConfig]]
   end
end

---load link colorscheme
---@param theme string?
local function load(theme)
   if vim.fn.has("nvim-0.7") ~= 1 then
      vim.notify("link.nvim: you must use neovim 0.7 or higher")
      return
   end
